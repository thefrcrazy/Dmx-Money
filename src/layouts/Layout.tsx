...
const Layout: React.FC<LayoutProps> = ({ children, activePage, setActivePage }) => {
  ...
  return (
    <div className="flex h-screen w-screen text-gray-900 dark:text-gray-100 font-sans overflow-hidden bg-[var(--color-bg-primary)] dark:bg-[var(--color-bg-primary)]">
      ...
      {/* Sidebar */}
      <aside className={`flex-shrink-0 flex flex-col bg-[var(--color-bg-secondary)] dark:bg-[var(--color-bg-secondary)] border-r border-black/[0.05] dark:border-white/10 z-40 transition-all duration-300 ${isCollapsed ? 'w-[82px]' : 'w-56'}`}>
        ...
        <div className="p-4 border-t border-black/[0.05] dark:border-white/10 overflow-x-hidden">
          <div className="space-y-0.5">
            <button
              ...
              onMouseEnter={(event) => showSidebarTooltip('Catégories', event.currentTarget)}
              onMouseLeave={hideSidebarTooltip}
              onFocus={(event) => showSidebarTooltip('Catégories', event.currentTarget)}
              onBlur={hideSidebarTooltip}
            ...
            >
              ...
              {!isCollapsed && <span className="min-w-0 truncate">Catégories</span>}
            </button>
            <button
              onClick={() => setActivePage('settings')}
              onMouseEnter={(event) => showSidebarTooltip('Paramètres', event.currentTarget)}
              onMouseLeave={hideSidebarTooltip}
              onFocus={(event) => showSidebarTooltip('Paramètres', event.currentTarget)}
              onBlur={hideSidebarTooltip}
            ...
            >
              ...
              {!isCollapsed && <div className="min-w-0 flex-1 flex justify-between items-center">
                  <span className="min-w-0 truncate">Paramètres</span>
                  {updateAvailable && (
